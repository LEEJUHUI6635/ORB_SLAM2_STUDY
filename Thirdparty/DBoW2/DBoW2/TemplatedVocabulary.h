...
// transform(vCurrentDesc,mBowVec,mFeatVec,4)
// TDescriptor와 F의 템플릿 인수를 가지는 클래스 템플릿을 선언
template<class TDescriptor, class F> 
void TemplatedVocabulary<TDescriptor,F>::transform(
  const std::vector<TDescriptor>& features,
  BowVector &v, FeatureVector &fv, int levelsup) const
{
  v.clear(); // BoW vector 초기화
  fv.clear(); // feature vector 초기화
  
  if(empty()) // safe for subclasses -> returns whether the vocabulary is empty(학습x)
  {
    return; // vocabulary가 비어있다면 empty() = true, 비어있지 않다면 empty() = false
  }
  
  // normalize 
  LNorm norm;
  bool must = m_scoring_object->mustNormalize(norm); // normalization이 수행되어야 하면 true, 수행되어야 하지 않는다면 false
  
  typename vector<TDescriptor>::const_iterator fit;
  
  if(m_weighting == TF || m_weighting == TF_IDF) // scoring을 하는 방식이 TF or TF_IDF라면,
  {
  ...
  if(must) v.normalize(norm); // must = true -> L1-normalization
}
