...
{   // return Mat(*this, Range(startrow, endrow), Range::all()); -> 같은 class에 속한 함수가 return을 자기 자신의 class로 받기 때문에 중복하여 .을 찍고 함수의 호출이 가능하다.
    ...
    mtcw = mTcw.rowRange(0,3).col(3); // rowRange() -> 관심 영역, translation vector
...
cv::Mat Frame::UnprojectStereo(const int &i) // i -> 하나의 keyframe 내의 keypoint index
{
